"""
MediaMarkt.de 价格提取测试脚本
用法: python test_mediamarkt.py
...
def clean_price(text):
    ...
    clean_text = text.replace("€", "").replace("£", "").replace("EUR", "").replace("\xa0", "").strip()
    if currency == "EUR":
...
async def test_mediamarkt_price():
    # 使用一个真实的 MediaMarkt TV 产品页（Samsung QLED TV 示例）
    # 可在运行前替换为任意真实的 TV 产品 URL
    test_url = "https://www.mediamarkt.de/search?query=samsung+qled+tv"
    ...
    async with async_playwright() as p:
        ...
        # 尝试接受 Cookie
        for btn in ["[data-testid='mms-accept-all-button']", "button:has-text('Alle akzeptieren')", "#onetrust-accept-btn-handler"]:
            try:
        ...
        with open("debug_mediamarkt_search.html", "w", encoding="utf-8") as f:
            f.write(content[:20000])
        print("[测试] 已保存页面 HTML 到 debug_mediamarkt_search.html")
        ...
        if links:
            ...
            if first_href:
                ...
                with open("debug_mediamarkt_product.html", "w", encoding="utf-8") as f:
                    f.write(prod_content[:30000])
                print("[测试] 已保存产品页 HTML 到 debug_mediamarkt_product.html")

        await browser.close()
        print("\n[测试] 完成！")

if __name__ == "__main__":
    asyncio.run(test_mediamarkt_price())
